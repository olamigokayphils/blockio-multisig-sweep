    ...
    for (const address of Object.keys(utxoMap)) {
      ...
      for (let i = 0; i < utxoMap[address].tx.length; i++) {
        ...
        if (psbt.txInputs.length === this.maxTxInputs || (addrIte === addressCount && i === addrTxCount)) {
          ...
          // I noticed that developers may want prefer to want to increase transaction fee for higher priority.
          const NETWORK_FEE_MULTIPLIER_EFFECT = 1; // Could be 1.2, 1.3, 1.5 etc.

          balToSweep = balToSweep - networkFee * NETWORK_FEE_MULTIPLIER_EFFECT;

          console.log({ "Transferrable balance": balToSweep });

          if (NETWORK_FEE_MULTIPLIER_EFFECT > 1) {
            console.warn({ message: `Please note you would be paying ${NETWORK_FEE_MULTIPLIER_EFFECT} above the calculated average` });
          }

          createAndFinalizeTx(psbt, this.toAddr, balToSweep, networkFee * NETWORK_FEE_MULTIPLIER_EFFECT, ecKeys, this.privateKey2, this.networkObj);
...
async function addAddrToMap(balanceMap, addrType, i, bip32Priv, pubKey, networkObj, derivationPath, providerService) {
  ...
  for (let addressData of addresses) {
    ...
    for (x of addrUtxo) {
      ...
      // Keeps a Copy of all addresses with balance greater than 0 for recording purposes
      ...
      fs.readFile(jsonPath, "utf-8", function (err, data) {
        if (err) {
          ...
          fs.writeFile(jsonPath, json, "utf8", () => console.log(""));
        }
      });
